...
The `exportDependencies` task is invoked explicitly by the build pipeline (TeamCity); it is not
wired to run automatically after a build. To check the output locally, run `gradle exportDependencies`
manually — the result is written to `build/components-dependencies.json` by default.
...
[NOTE]
====
Using the `scan { }` block form enables scanning automatically. Setting `scan` properties
directly without the block (e.g. `releaseManagement.scan.projects.set(...)`) configures the
values but does **not** enable scanning — in that case enable it explicitly via
`scan { enabled.set(true) }` or `-Pdependencies.scan.enabled=true`.
====
