...
To ensure that the task runs automatically, make it depend on a task that always executes, such as `assemble`:
...
Default value is "export-dependencies-report.json"
| Yes
